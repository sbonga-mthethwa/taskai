...
import { Calendar, Users, CheckCircle2, Save, Pencil, CalendarDays, Search, ChevronDown, Paperclip } from "lucide-react";
...
const ItemDetailModal = ({ item, open, onClose, onSave }: ItemDetailModalProps) => {
  const { user } = useAuth();
  ...
  const canEdit = isTask
    ? permissions.canEditTask({ createdBy: item.createdBy, assignedUsers: item.assignedUsers })
  ...
  const dueDateDisplay = item.date
    ? `${item.date.toLocaleString("default", { month: "short" })} ${item.date.getDate()}, ${item.date.getFullYear()}`
    : dueDate || "—";
      ...
      <SheetContent side="right" className="w-full sm:w-[70vw] sm:max-w-[70vw] lg:w-[52vw] lg:max-w-[52vw] p-0 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
          {!canEdit && (
            <div className="rounded-xl border border-border bg-muted/20 px-4 py-3 text-sm text-muted-foreground">
              You have read-only access to this {item.type}.
            </div>
          )}

          <section className="space-y-4">
            <div className="space-y-2">
              <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">{item.type === "project" ? "Project overview" : "Task overview"}</p>
              {editingEnabled ? (
                <input
                  value={title}
          ...
          <section className="space-y-4 pt-1 border-t border-border">
            ...
            <DocumentAttachments attachedDocIds={attachedDocuments} onChange={setAttachedDocuments} editing={editingEnabled} entityId={item.id} />
          </section>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80 flex-shrink-0">
          ...
          <div className="flex gap-2">
            ...
            {canEdit && isTask && status !== "completed" && (
